...
class Engine(object):
	...
	def train(self):
		loss_epoch = 0.
		num_batches = 0
		red_light_loss = torch.nn.BCELoss()
		model.train()
		...
		for data in progress:
			...
			for i in range(config.seq_len):
				...
				seg.append(seg_in[i].to(args.device, dtype=torch.float32))
			...
			gt_tl = data['tl_gts'][0].to(args.device, dtype=torch.float32)
   
			# target point
			target_point = torch.stack(data['target_point'], dim=1).to(args.device, dtype=torch.float32)
			
			pred_wp, pred_tl = model(fronts+lefts+rights+rears, lidars, seg, target_point, gt_velocity, self.cur_epoch)
			...
			if pred_tl is not None:
				m = torch.nn.Sigmoid()
				tl_loss = red_light_loss(m(pred_tl), gt_tl.unsqueeze(1))
				total_loss = (loss+tl_loss)
				progress.set_postfix({'epoch': self.cur_epoch, 'tl': tl_loss.item(), 'wp': loss.item()})
			else:
				total_loss = loss
				progress.set_postfix({'epoch': self.cur_epoch, 'wp': loss.item()})
			total_loss.backward()
	...
	def validate(self):
		...
		with torch.no_grad():	
			...
			for batch_num, data in enumerate(tqdm(dataloader_val), 0):
				...
				pred_wp, _ = model(fronts+lefts+rights+rears, lidars, seg, target_point, gt_velocity, 100)
