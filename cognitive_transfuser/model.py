...
class GPT_Seg(nn.Module):
    ...
    def forward(self, image_tensor, lidar_tensor, seg, velocity):
        """
        Args:
            image_tensor (tensor): B*4*seq_len, C, H, W
            lidar_tensor (tensor): B*seq_len, C, H, W
        ...
        # pad token embeddings along number of tokens dimension
        token_embeddings = torch.cat([image_tensor, lidar_tensor, seg_tensor], dim=1).permute(0,1,3,4,2).contiguous()
        token_embeddings = token_embeddings.view(bz, -1, self.n_embd) # (B, an * T, C)
        ...
        image_tensor_out = x[:, :self.config.n_views*self.seq_len, :, :, :].contiguous().view(bz * self.config.n_views * self.seq_len, -1, h, w)
        lidar_tensor_out = x[:, self.config.n_views*self.seq_len:self.config.n_views*self.seq_len*2, :, :, :].contiguous().view(bz * self.seq_len, -1, h, w)
...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.seg_encoder = ImageCNN(512, normalize=True)
    ...
    def forward(self, image_list, lidar_list, seg_list, velocity):
        '''
        ...
        seg_channel = seg_list[0].shape[1]
        ...
        seg_tensor = torch.stack(seg_list, dim=1).view(bz * self.config.n_views * self.config.seq_len, seg_channel, h, w)
        ...
        seg_features = self.seg_encoder.features.conv1(seg_tensor)
        ...
        seg_features = self.seg_encoder.features.layer1(seg_features)

        # fusion at (B, 64, 64, 64)
        image_embd_layer1 = self.avgpool(image_features)
        lidar_embd_layer1 = self.avgpool(lidar_features)
        seg_embd_layer1 = self.avgpool(seg_features)

        image_features_layer1, lidar_features_layer1 = self.transformer1(image_embd_layer1, lidar_embd_layer1, seg_embd_layer1, velocity)
...
class FLC(object):
    def __init__(self, Ke=1.3, Kce=1.3, Ku=4.0):
        self._Ke = Ke
        self._Kce = Kce
        self._Ku = Ku
        ...
        self._max = 0.0
        self._min = 0.0
        self.controller = Fuzzy(Ke, Kce, Ku)
    ...
    def step(self, error):
        ...
        self._max = max(self._max, abs(error))
        self._min = -abs(self._max)
        prev_error = self._window[-1]
        k_error = self._Ke*error
        d_error = (error - prev_error)*self._Kce
        return self.controller.step(k_error, d_error)
...
class TransFuser(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        self.speed_flc = FLC(Ke = 1.3, Kce = 1.3, Ku = 4.0)

        self.encoder = Encoder(config).to(self.device)

        self.tl_module = nn.Sequential(
                    nn.Linear(512, 256),
    ...
    def forward(self, image_list, lidar_list, seg, target_point, velocity, epoch):
        '''
        ...
        Args:
            ...
            velocity (tensor): input velocity from speedometer
        '''
        fused_features = self.encoder(image_list, lidar_list, seg, velocity)
        z = self.join(fused_features)
        if epoch < 10:
                pred_tl = self.tl_module(fused_features)
        else:
            pred_tl = None
        output_wp = list()
    ...
    def control_pid(self, waypoints, velocity):
        ...
        flc_throttle = self.speed_flc.step(delta)
        flc_throttle = np.clip(flc_throttle, 0.0, self.config.max_throttle)
        throttle = flc_throttle if not brake else 0.0
        ...
        print(f'pid throttle: {throttle}')
        print(f'flc throttle: {flc_throttle}')
