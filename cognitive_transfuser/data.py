...
class CARLA_Data(Dataset):
    ...
    def __init__(self, root, config):
        ...
        self.seg_dir = config.seg_dir #hs
        ...
        for sub_root in tqdm(root, file=sys.stdout):
            ...
            sub_root_name = sub_root.split('/')[-1]
            # dump to npy if no preload
            if not os.path.exists(preload_file):
            # if True: #hs # make preload file again if needed
                preload_front = []
                preload_left = []
                preload_right = []
                preload_rear = []
                ...
                for route in routes:
                    ...
                    seg_dir = os.path.join(self.seg_dir, sub_root_name, route) #hs
                    print(route_dir)
                    ...
                    for seq in range(num_seq):
                        ...
                        for i in range(self.seq_len):
                            ...
                            segs.append(seg_dir+"/rgb_front/"+filename[:-3]+'npy') #hs
                            seg_gts.append(route_dir+"/seg_front/"+filename)
                            jsons.append(route_dir+"/measurements/"+filename[:-3]+'json')
                        ...
                        preload_seg.append(segs) #hs
                        preload_seg_gt.append(seg_gts)
                        preload_json.append(jsons)
                        preload_x.append(xs)
                        preload_y.append(ys)
                ...
                preload_dict['seg'] = preload_seg #hs
                preload_dict['json'] = preload_json
                preload_dict['seg_gt'] = preload_seg_gt
            ...
            self.seg += preload_dict.item()['seg']#hs
            self.seg_gt += preload_dict.item()['seg_gt']
            self.json += preload_dict.item()['json']
    ...
    def __getitem__(self, index):
        ...
        seq_seg_gts = self.seg_gt[index] #hs
        seq_jsons = self.json[index]
        seq_x = self.x[index]
        seq_y = self.y[index]
        ...
        for i in range(self.seq_len):
            ...
            # data['seg_gts'].append(F.one_hot(torch.from_numpy(np.array(
            #     scale_and_crop_image(Image.open(seq_seg_gts[i]), scale=self.scale, crop=self.input_resolution, is_seg_gt=True))).to(dtype=torch.int64),28).permute(2,0,1))
            temp = np.load(seq_segs[i])
            temp = np.repeat(temp, repeats=3, axis=2)
            data['segs'].append(torch.from_numpy(temp).permute(2,0,1))
            data['seg_gts'].append(torch.from_numpy(np.array(
                scale_and_crop_image(Image.open(seq_seg_gts[i]), scale=self.scale, crop=self.input_resolution, is_seg_gt=True))))
            ...
            tl_json_data = json.load(open(seq_jsons[i],'r'))            
            data['tl_gts'].append(tl_json_data['is_red_light_present'])
            # fix for theta=nan in some measurements
...
def scale_and_crop_image(image, scale=1, crop=256, is_seg_gt=False):
    """
