...
sys.path.insert(0,'./10.TS_aux_trans1/')
from leaderboard.autoagents import autonomous_agent
from model import TransFuser
from config import GlobalConfig
from data import scale_and_crop_image, lidar_to_histogram_features, transform_2d_points
from team_code.planner import RoutePlanner
sys.path.insert(0,'../STDC-Seg')
...
class TransFuserAgent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self.input_buffer = {'seg_rgb':deque(), 'rgb': deque(), 'rgb_left': deque(), 'rgb_right': deque(), 
		...
		n_classes = 19
		self.seg_net = BiSeNet(backbone='STDCNet1446', n_classes=n_classes,
		use_boundary_2=False, use_boundary_4=False, 
		use_boundary_8=True, use_boundary_16=False, 
		use_conv_last=False)
		self.seg_net.load_state_dict(torch.load('../STDC-Seg/checkpoints/STDC2-Seg/model_maxmIOU75.pth'))
		self.seg_net.cuda()
		self.seg_net.eval()
	...
	def make_segmentation(self, img):
		scale = 0.75
		transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),
            ])
		img = transform(img).to('cuda').unsqueeze(0)
		N, C, H, W = img.size()
		new_hw = [int(H*scale), int(W*scale)]
		img = F.interpolate(img, new_hw)
		logits = self.seg_net(img)[0]
		logits = F.interpolate(logits, size=[H,W])
		preds = torch.argmax(logits, dim=1)
		preds = preds.expand([3,H,W]) #3*300*400
		preds = preds[:,int(H/2)-128:int(H/2)+128,int(W/2)-128:int(W/2)+128].unsqueeze(0).to(dtype=torch.float32)
		return preds
	...
	def run_step(self, input_data, timestamp):
		...
		if self.step < self.config.seq_len:
			...
			seg_rgb = Image.fromarray(tick_data['rgb'])
		...
		if(self.step  % 2 == 0 or self.step <= 4):
			...
			seg = self.make_segmentation(seg_rgb)
			## Waypoint prediction
			self.pred_wp, _ = self.net(self.input_buffer['rgb'] + self.input_buffer['rgb_left'] + \
							   self.input_buffer['rgb_right']+self.input_buffer['rgb_rear'], \
							   self.lidar_processed, [seg], target_point, gt_velocity, 100)
